...
export function createCampaign({ name, clientId = null, squadId = null, notes = "", status = "ACTIVE" } = {}) { const timestamp = now(); return { id: createId("campaign"), name: String(name ?? "").trim(), clientId: clientId || null, squadId: squadId || null, notes: String(notes ?? "").trim(), status, startedAt: timestamp, endedAt: null, createdAt: timestamp, updatedAt: timestamp }; }
