...
const gapReport = (gaps) => {
  ...
  const rows = gaps.map((gap) => `<tr><td><strong>${escapeHtml(formatPhone(gap.phone))}</strong><span class="table-secondary">${escapeHtml(gap.identification || "Sem identificação")}</span></td><td>${escapeHtml(gap.clientName)}</td><td>${escapeHtml(gap.campaignName)}</td><td><div class="table-actions"><button class="button button-quiet" data-action="open-number" data-id="${gap.numberId}">Ver número</button></div></td></tr>`).join("");
  return `<details class="gap-report" data-admin-only><summary>⚠ ${gaps.length} possíve${gaps.length === 1 ? "l" : "is"} vínculo${gaps.length === 1 ? "" : "s"} faltando</summary><p class="form-hint">Número já tem o cliente associado, mas nunca teve vínculo (nem já encerrado) com esta campanha ativa desse cliente. Pode ser proposital, ou uma associação que ficou faltando de antes de números aceitarem várias campanhas ao mesmo tempo. Revise e vincule manualmente quando fizer sentido — depois de revisado (vinculado ou o vínculo encerrado de propósito), esta combinação não volta a aparecer aqui.</p><div class="table-scroll"><table><thead><tr><th>Número</th><th>Cliente</th><th>Campanha sem vínculo</th><th>Ação</th></tr></thead><tbody>${rows}</tbody></table></div></details>`;
};
...
const externalLinkRow = (link) => {
  ...
  return `<tr class="is-external-row"><td><strong>${escapeHtml(formatPhone(link.phoneNormalized))}</strong><span class="badge-external">Não é nosso${link.companyLabel ? ` — ${escapeHtml(link.companyLabel)}` : ""}</span></td><td>${link.clientAccountLabel ? `Conta do Cliente: ${escapeHtml(link.clientAccountLabel)}` : "—"}</td><td>—</td><td>—</td><td>—</td><td>${date(link.linkedAt)}</td><td>${link.endedAt ? date(link.endedAt) : currentActions}</td></tr>`;
};
