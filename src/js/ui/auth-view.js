...
export function renderRegistration(squads = [], message = "") {
  return authShell(`${authMobileBrand()}<p class="eyebrow">Novo acesso</p><h1>Criar conta</h1><p class="auth-intro">Comece com acesso User. Admin só pode ser concedido posteriormente por um administrador.</p>${message ? `<div class="auth-message" role="alert">${escapeHtml(message)}</div>` : ""}<form data-auth-form="register"><div class="auth-form-grid"><label>Nome<input class="input" name="name" autocomplete="name" placeholder="Nome completo" required></label><label>E-mail<input class="input" name="email" type="email" autocomplete="email" placeholder="voce@empresa.com" required></label>${passwordField("Senha", "password", { autocomplete: "new-password", minlength: "8", placeholder: "Mínimo de 8 caracteres" })}<label>Cargo<select class="input" name="jobTitle" required><option value="">Selecione</option><option value="ANALYST">Analista</option><option value="ACCOUNT_MANAGER">Gerente de Contas</option></select></label><label class="auth-form-wide">Squad<select class="input" name="squadId"><option value="">Sem Squad</option>${squadOptions(squads)}</select></label></div><button class="button button-primary auth-submit" type="submit">Cadastrar com segurança <span>→</span></button><button class="button button-quiet" type="button" data-auth-mode="login">Voltar ao login</button></form>`);
}
