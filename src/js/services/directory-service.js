...
export class DirectoryService {
  ...
  update(type, id, input) { const item = this.numbersService.state[type].find((entry) => entry.id === id); if (!item) throw new Error("Registro não encontrado."); const name=String(input.name??"").trim();if(!name)throw new Error("Informe o nome.");Object.assign(item, { name, ...(type === "responsibles" ? { team: String(input.team ?? "").trim() } : {}), ...(type === "clients" ? { squadId:input.squadId||null } : {}), updatedAt: now() }); this.numbersService.persist(); return item; }
