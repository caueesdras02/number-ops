export class SupabaseAuthRepository {
  ...
  async signUp(payload) { const options = { ...payload.options, emailRedirectTo: window.location.origin + window.location.pathname }; const { data, error } = await this.client.auth.signUp({ ...payload, options }); if (error) throw error; return data; }
  ...
  async checkSignupAuthorization(email) { const { data, error } = await this.client.rpc("check_signup_authorization", { p_email: email }); if (error) throw error; return Boolean(data); }
