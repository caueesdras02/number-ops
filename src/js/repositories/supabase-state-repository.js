...
export class SupabaseStateRepository {
  ...
  async sync(state) {
    ...
    await upsert("campaigns",state.campaigns.map((x)=>({id:x.id,name:x.name,client_id:x.clientId,squad_id:x.squadId,responsible_id:x.responsibleId||null,notes:x.notes||"",status:x.status,started_at:x.startedAt,ended_at:x.endedAt||null,created_at:x.createdAt,updated_at:x.updatedAt})));
