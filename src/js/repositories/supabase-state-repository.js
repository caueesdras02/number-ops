...
export class SupabaseStateRepository {
  ...
  static async create(client) {
    ...
    const squadsByNumber=groupBy(rows.number_squads,(item)=>item.number_id);
    const restrictionsByNumber=groupBy(rows.restrictions.filter((item)=>!item.ended_at),(item)=>item.number_id);
  ...
  /** Remoção definitiva de um registro (usado só pela exclusão MASTER, com RLS current_is_master()). */
  async remove(table,id) { check(await this.client.from(table).delete().eq("id",id)); }

  async sync(state) {
    // Envia apenas as linhas alteradas nesta sessão (comparado ao último snapshot
    // sincronizado). Evita que um save de aba defasada reescreva campos que outra
    // aba já mudou (ex.: client.squad_id).
    const changed=(previousList,nextList,mapRow)=>{
      const prev=new Map((previousList??[]).map((item)=>{const row=mapRow(item);return [row.id,JSON.stringify(row)];}));
      return (nextList??[]).map(mapRow).filter((row)=>prev.get(row.id)!==JSON.stringify(row));
    };
    ...
    const campaignRow=(x)=>({id:x.id,name:x.name,client_id:x.clientId,squad_id:x.squadId,responsible_id:x.responsibleId||null,notes:x.notes||"",status:x.status,stage:x.stage||"CAPTACAO",started_at:x.startedAt,ended_at:x.endedAt||null,created_at:x.createdAt,updated_at:x.updatedAt});
    ...
    const incidentRow=(x)=>({id:x.id,number_id:x.numberId,type:x.type,title:x.title,description:x.description||"",status:x.status,responsible_id:x.responsibleId||null,resolution_notes:x.resolutionNotes||"",resolved_by_id:x.resolvedById||null,resolved_at:x.resolvedAt||null,created_at:x.createdAt,updated_at:x.updatedAt});
    const historyRow=(x)=>({id:x.id,number_id:x.numberId,type:x.type,description:x.description,previous_value:x.previousValue??null,new_value:x.newValue??null,metadata:x.metadata??null,occurred_at:x.occurredAt});

    await upsert("squads",changed(this.previous.groups,state.groups,squadRow));
    await upsert("clients",changed(this.previous.clients,state.clients,clientRow));
    await upsert("responsibles",changed(this.previous.responsibles,state.responsibles,responsibleRow));
    await upsert("locations",changed(this.previous.locations,state.locations,locationRow));
    await upsert("numbers",changed(this.previous.numbers,state.numbers,numberRow));
    ...
    await upsert("campaigns",changed(this.previous.campaigns,state.campaigns,campaignRow));
    await upsert("number_campaign_links",changed(this.previous.numberCampaignLinks,state.numberCampaignLinks,linkRow));
    await upsert("incidents",changed(this.previous.incidents,state.incidents,incidentRow));
    await this.syncRestrictions(state);
    await upsert("history_events",changed(this.previous.historyEvents,state.historyEvents,historyRow));
