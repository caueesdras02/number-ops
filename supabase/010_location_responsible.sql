-- 010 — Colaborador responsável por Localização (dispositivo). Incremental e
-- não destrutivo: adiciona locations.responsible_id e estende a função de
-- auditoria (copiada de 009, só o ramo `locations` muda) para registrar a
-- troca de colaborador. Aplique depois de 009_master_permissions_and_cleanup.sql.

begin;

-- ---------------------------------------------------------------------------
-- 1. Localização -> Colaborador (FK opcional; on delete set null como as demais)
-- ---------------------------------------------------------------------------
alter table public.locations
  add column if not exists responsible_id text references public.responsibles(id) on delete set null;
create index if not exists locations_responsible_idx on public.locations (responsible_id);

-- ---------------------------------------------------------------------------
-- 2. Auditoria: LOCATION_RESPONSIBLE_CHANGED (cópia de 009 + um ramo novo)
-- ---------------------------------------------------------------------------
