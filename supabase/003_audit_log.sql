...
begin
  ...
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='audit_logs' and column_name='table_name')
     and not exists(select 1 from information_schema.columns where table_schema='public' and table_name='audit_logs' and column_name='entity_type') then
    alter table public.audit_logs rename column table_name to entity_type;
  end if;
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='audit_logs' and column_name='record_id')
     and not exists(select 1 from information_schema.columns where table_schema='public' and table_name='audit_logs' and column_name='entity_id') then
    alter table public.audit_logs rename column record_id to entity_id;
  end if;
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='audit_logs' and column_name='old_data')
     and not exists(select 1 from information_schema.columns where table_schema='public' and table_name='audit_logs' and column_name='previous_data') then
    alter table public.audit_logs rename column old_data to previous_data;
  end if;
end $$;
...
drop index if exists public.audit_logs_table_record_idx;
...
begin
  ...
  insert into public.audit_logs(user_id,action,entity_type,entity_id,previous_data,new_data,metadata)
  values(auth.uid(),p_action,p_entity_type,p_entity_id,p_previous_data,p_new_data,coalesce(p_metadata,'{}'::jsonb));
end;
$$;

revoke all on function public.append_audit_log(text,text,text,jsonb,jsonb,jsonb) from public, anon, authenticated;
...
begin
  foreach v_table in array array['numbers','restrictions','number_clients','number_squads','campaigns','number_campaign_links','clients','squads','incidents','profiles'] loop
    execute format('drop trigger if exists %I on public.%I','audit_'||v_table||'_changes',v_table);
    execute format('create trigger %I after insert or update or delete on public.%I for each row execute function public.capture_number_ops_audit()','audit_'||v_table||'_changes',v_table);
  end loop;
end $$;
...
using(public.current_access_level()='ADMIN');
