...
-- Incremental: adds a responsible collaborator to campaigns. Additive only, no data loss.
alter table public.campaigns add column if not exists responsible_id text references public.responsibles(id) on delete set null;
create index if not exists campaigns_responsible_idx on public.campaigns (responsible_id);

-- Replaces the audit trigger function (logic only, no data touched) to also capture
-- campaign reactivation and responsible changes. Everything else is unchanged from 003.
...
-- Function replaced in place; existing triggers on public.campaigns etc. already point to it, no re-creation needed.
